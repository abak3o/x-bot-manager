...
// 戻るリンクにアカウントIDを設定
document.getElementById('backToAccount').href = `account_detail.html?account_id=${accountId}`;
...
function generateHours() {
    ...
    if (!interval) {
        document.getElementById('hoursGrid').innerHTML = '<p style="color:#999;">間隔を選択してください</p>';
        selectedHours = [];
        return;
    }
...
async function loadSchedules() {
    try {
        ...
        if (schedules.length === 0) {
            list.innerHTML = '<p class="empty-message">スケジュール設定がありません</p>';
            return;
        }
            ...
            <div class="schedule-card ${schedule.is_active ? '' : 'inactive'}">
                <div class="schedule-header">
                    ...
                    <div class="schedule-actions">
                        <button onclick="toggleScheduleActive(${schedule.id}, ${!schedule.is_active})" 
                                class="btn-small ${schedule.is_active ? 'btn-warning' : 'btn-success'}">
                            ${schedule.is_active ? '無効化' : '有効化'}
                        </button>
                        <button onclick="editSchedule(${schedule.id})" class="btn-small">編集</button>
                        <button onclick="deleteSchedule(${schedule.id})" class="btn-small btn-danger">削除</button>
                    </div>
                </div>
                <div class="schedule-hours">
                    ${schedule.hours.map(h => `<span class="hour-tag">${h}</span>`).join('')}
                </div>
                <div class="schedule-info">
                    <small>作成: ${new Date(schedule.created_at).toLocaleString('ja-JP')}</small>
                </div>
            </div>
        `).join('');
...
async function toggleScheduleActive(scheduleId, isActive) {
    try {
        const response = await fetch(`/accounts/${accountId}/hourly-schedules/${scheduleId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
